...
export class ProcessMessageUseCase {
  ...
  async process(message: string, role: string, tokens: number) {
    ...
    const conversation = await this.conversationRepository.saveConversation(message, user_id);
  ...
  async execute(summary: string, messageText: string, sender: string, user_id: UUID) {
    ...
    // Guardar la conversación en la base de datos
    const conversation = await this.conversationRepository.saveConversation(summary, user_id);
